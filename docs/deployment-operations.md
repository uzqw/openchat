...
- `~/.opencli/clis/<site>` 不存在（`<site>` = `OPENCLI_SITE`，即 gemini）且没有已安装 OpenCLI plugin；否则拒绝写操作。
...
2. **Bridge/版本/登录**：启动期做一次带专用 profile 的 version/doctor/status/whoami 探针（另含 models），探针与 opencli daemon 背靠背瞬时失败时 3s 后重试一次（`probeWithRetry`；`opencli --version` 必须为锁定的 v1.8.7，Extension 为 v1.0.23）；此后 `GET /api/providers` 一律返回**缓存**，**不能因 UI 轮询反复入队**。
...
| `OPENCLI_SITE` | `gemini` | | 站点固定为 `gemini`（唯一 provider）；其他值启动时校验拒绝（fail closed） |
...
| `OPENCLI_NTFY_URL` | `""` | | 回答完成推送：非空时每次 Gemini 回答成功即向该 ntfy 发布地址推送一条通知（每个部署自建 topic 的完整 URL，如 `https://ntfy.sh/<topic>`；title=提问前 10 字，message=回答前 15 字，best-effort，失败仅记日志不阻塞队列）；留空关闭 |
...
时写操作（ask/retry/login）一律拒绝（`internal/provider/guard.go`），
探针按站点跑（`probeWithRetry`）。
