...
- `~/.opencli/clis/<site>` 不存在（`<site>` = `OPENCLI_SITE`，gemini 或 grok）且没有已安装 OpenCLI plugin；否则拒绝该站点写操作。
...
- 用户已在该专用 profile 的 Gemini/Grok Web 人工登录。
...
2. **Bridge/版本/登录（双站点）**：启动期对 gemini 与 grok 各自做一次带专用 profile 的 version/doctor/status/whoami 探针（gemini 另含 models），探针与 opencli daemon 背靠背瞬时失败时 3s 后重试一次（`probeWithRetry`；`opencli --version` 必须为锁定的 v1.8.7，Extension 为 v1.0.23）；此后 `GET /api/providers` 一律返回**双站点缓存**，**不能因 UI 轮询反复入队**。
3. **按需刷新**：探针只在启动时与用户点击「检测在线」（`POST /api/providers/{site}/refresh`）时按站点运行，**无后台定时轮询**；active 有成功 turn 后暂停该站点所有非 ask 的 OpenCLI operation，只能读缓存，避免导航 shared tab。
4. **隔离（按站点）**：任一 ask 进入 `unknown_outcome` 后立即归档 active conversation 并将**该站点**置为持久化隔离；隔离期间该站点的所有 OpenCLI operation（含按需刷新与 login）暂停，GET 只返回缓存，直到用户在可见 Chrome 确认已停止生成并通过 `POST /api/tasks/{id}/acknowledge-unknown` 解除。
...
| `OPENCLI_SITE` | `gemini` | | 新建会话的**默认站点**：`gemini` 或 `grok`（不是全局切换；历史会话保留各自 `provider`，恢复/追问按会话自己的站点；grok 无 models 命令、ask 不支持 `--model/--thinking`，模型/思考选择自动隐藏；启动时校验，非法值拒绝启动）。生产 = `grok` |
...
| `OPENCLI_TIMEOUT_SECONDS` | `300` | | provider ask 超时（kill 上限，同时传给 `opencli --timeout`） |
...
时**该站点**的写操作（ask/retry/login）一律拒绝，双站点各自校验
（`internal/provider/guard.go`），探针按站点跑（`probeWithRetry`）。
