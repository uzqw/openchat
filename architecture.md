...
> 当前版本：Gemini 单一 provider。
> OpenCLI 基线：`@jackwener/opencli` **v1.8.7**、Browser Bridge Extension **v1.0.23**

## 1. 目标

平台最终目标是通过用户已经登录的真实 Chrome，会聚多个 AI 网页版；**当前版本只接入 Gemini**：
每个会话在创建时写入 `provider` 字段（恒为 `gemini`），恢复与追问都走 Gemini 适配器；
`OPENCLI_SITE` 仅作兼容保留，必须是 `gemini`：
...
- 单机、单用户、单后端实例。
- 单一 provider：仅 gemini（Gemini）；`OPENCLI_SITE` 只接受 `gemini`，其他值启动即拒（fail closed）。ChatGPT、Kimi、DeepSeek 后置。
- 站点能力收敛在 `internal/opencli/site.go` 的 Site 能力表（子命令名、--model/--thinking、models 命令、sentinel、会话 URL 形状）；仅保留 gemini 一条目。
- 当前只有一个 active conversation（新建/恢复会归档旧的）。
- 当前活跃会话支持连续追问；旧会话可恢复续聊（每个会话保存其站点远端会话 id，恢复时按会话自己的站点 `detail` 导航后继续）。
- 无 `remote_id` 的旧会话保持只读（gemini 为 `/app/<id>`）。
...
| Provider 范围 | Gemini 单一 provider；`OPENCLI_SITE` 固定为 `gemini`，非法值拒绝启动 |
...
| 文档 | 内容 |
|---|---|
| [`docs/opencli-contract.md`](docs/opencli-contract.md) | Gemini v1.8.7 命令、输出和错误合同 |
